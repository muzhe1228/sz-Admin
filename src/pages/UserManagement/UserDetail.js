...
export default class UserList extends Component {
    ...
    saveState = (status) => {
        ...
        //判断是否输入值和审核通过都不用加输入值
        if (this.state.value1.length > 0 || status == 3) {
            let obj = {
                seniorAuth: status,
                userId: this.state.itemData.userId,
                remark:this.state.value1
            }
            console.log(obj);
                ...
                <div style={{ display: 'flex', flexDirection: 'row', minHeight: '900px' }}>
                    <div style={{ flex: 1, flexDirection: 'column' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                            ...
                            {/* 限制40个字 */}
                            <div className="shenHeIpt">
                                请输入审核失败的原因：
                                <TextArea placeholder="请输入审核失败的原因：" rows={6} onChange={(e) => this.changeEvent(e)} ref="input" maxLength="40" />
                            </div>
                        </div>
                        <div style={{ marginTop: 75, marginLeft: 100 }}>
                            <Button onClick={this.saveState.bind(this, 3)} disabled={this.state.disable} size='large' type='primary'
                                style={{ marginRight: 30 }}>审核通过</Button>
                            <Button onClick={this.saveState.bind(this, 2)} size='large' style={{ marginRight: 30 }}
                                type='danger'>审核不通过</Button>
                            <Button onClick={() => history.goBack()} size='large' >返回</Button>
                        </div>
                    </div>
                    <div style={{ flex: 1 }}>
                        <h2>认证信息</h2>
                        ...
                        <div>
                            ...
                            <div className='detail-item' style={{ width: '360px' }}>
                                <p className='text-margin-d'>手持证件照：</p>
                                <img onClick={() => this.setState({
                                    showModal: true,
                                    showImg: itemData.identityPictureThree ? itemData.identityPictureThree : img
                                })}
                                    src={itemData.identityPictureThree ? itemData.identityPictureThree : img}
                                    style={{ width: '100px', height: '100px', }}></img>
                            </div>
                            {/*<div className='detail-item' style={{width: '360px'}}>*/}
